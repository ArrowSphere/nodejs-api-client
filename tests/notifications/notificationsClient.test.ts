...
describe('NotificationsClient', () => {
  ...
  describe('create', () => {
    it('should create notification', async () => {
      let body_passed;
      nock(NOTIFICATIONS_URL)
        .post(TEST_NOTIFICATIONS_URL, (body) => {
          body_passed = body;
          return true;
        })
        .reply(200, PAYLOAD_ONE_NOTIFICATION);

      const payload: CreatePayloadType = {
      ...
      expect(body_passed).to.be.deep.equals(payload);
      expect(result).to.be.instanceof(GetResult);
  ...
  describe('getNotification', () => {
    it('should get one notification', async () => {
      ...
      const result = await client.getNotification('XXXXXXXXXXX');
      expect(result).to.be.instanceof(GetResult);
      expect(result.toJSON()).to.be.deep.equals(PAYLOAD_ONE_NOTIFICATION);
    });
  });
  ...
  describe('deleteNotification', () => {
    it('should list notification without parameters', async () => {
      ...
      await client.deleteNotification('XXXXXXXXXX');
      expect(nock.isDone()).to.be.true;
    });
  });

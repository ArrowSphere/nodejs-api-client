...
export class CustomersClient extends AbstractRestfulClient {
  ...
  public async createCustomer(
    ...
    try {
      ...
      const response: APIResponseResourceCreated = {
        status: 201,
        data,
      };
      ...
      const response: APIResponseError = {
        status: get(error, 'httpCode', get(error, 'code', 500)),
        message: get(
          error,
          'httpError',
          get(error, 'message', 'An error has occurred'),
        ),
      };
  ...
  public async updateCustomer(
    ...
    try {
      const { data } = await this.patch<AxiosResponse['data']>(
        payload,
        parameters,
      );
      const response: APIResponseCustomerUpdated = {
        status: 200,
        data,
      };

...
export class LicenseRequestResult extends AbstractEntity<LicenseRequestResultType> {
  readonly #licenseRequests?: LicenseRequest[];

  public constructor(input: LicenseRequestResultType) {
    super(input);

    this.#licenseRequests = input[
      LicenseRequestResultFields.COLUMN_LICENSE_REQUEST
    ]?.map(
      (result: LicenseRequestType): LicenseRequest =>
        new LicenseRequest(result),
    );
  }
  ...
  public toJSON(): LicenseRequestResultType {
    return {
      [LicenseRequestResultFields.COLUMN_LICENSE_REQUEST]: this.licenseRequests?.map(
